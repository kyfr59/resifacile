...
use App\Mail\PNDRecieved;

class HandleUndeliveredMailReceived implements ShouldQueue
{
    ...
    public function handle(MailevaService $mailevaService)
    {
        ...
        if($sending->status == SendingStatus::UNDELIVERED) {
            return response('', 201);
        }

        try {
            $pdf = $mailevaService->storePND($sending);
        } catch (\Exception $e) {
            throw new MailevaException(
                message  : "Impossible de stocker la preuve de non-distribution sur le serveur",
        ...
        try {
            Mail::to($sending->customer->email)->send(new PNDRecieved($sending, $pdf));

            $sending->update([
                'status' => SendingStatus::UNDELIVERED,
                'processed_at' => now(),
            ]);

        } catch (\Exception $e) {
            throw new MailevaException(
                message  : "Impossible d'envoyer le mail de notification du statut UNDELIVERED, le statut n'a pas été mis à jour",
                context  : self::class . '::' . __FUNCTION__,
                extraData: [
                    'sending_id'   => $sending->id,
                    'customer_id'  => $sending->customer_id,
