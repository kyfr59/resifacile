            ...
            <label class="mb-2 flex flex-col select">
                ...
                >
                    @foreach(\App\Enums\AddressType::cases() as $addressType)
                        <option
                            value="{{ $addressType }}" {{ ($addressType->value === $recipient['type']) ? 'selected' : '' }}>{{ $addressType->label() }}</option>
                    @endforeach
                </select>
            </label>
            @if(!$loop->first)
        ...
        <div>
            <div class="flex flex-col select">
                <label
                    ...
                    class="mb-2 text-xs text-blue-700 flex gap-2">Pays @error('recipients.' . $index . '.country') <span class="text-red-500 italic inline-flex px-2 bg-red-50">requis</span> @else *@enderror</label>
                <select
                    wire:model.defer="recipients.{{ $index }}.country"
                    id="recipients_{{ $index }}_country"
                    class="appearance-none w-full px-3 border rounded-[7px] outline-none h-14 md:h-8 placeholder:text-blue-300 bg-transparent @error('recipients.' . $index . '.country') border-red-500 @else border-blue-300 @enderror"
                >
