<div>
    @if (session()->has('unsubscribe-message'))
    ...
    @else
        @if ($subscriptionIsActive)
            ...
            >
                <div class="w-full md:w-auto bg-red-50 text-center text-red-500 rounded-[7px] p-2 leading-tight text-sm md:text-base">Vous ne bénéficierez plus dans avantages Accès+ !</div>
                    <div>
                        <button wire:click="$set('showModal', true)" class="bg-red-600 text-white px-4 py-2 rounded">
                        Résilier mon abonnement Accès+
                    </button>

                    <div x-data="{ open: @entangle('showModal') }">
                        <div x-show="open" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
                            <div class="bg-white rounded-lg p-6 w-96">
                                <h2 class="text-xl font-bold mb-4">Confirmer la résiliation</h2>
                                <p class="mb-4">Êtes-vous sûr de vouloir résilier votre abonnement Accès+ ? Cette action est irréversible.</p>
