<x-layouts.app>
    <x-slot:head>
        <title>{{ $entry->seo_title }}</title>
        <meta name="description" content="{{ $entry->seo_description }}">
        <meta name="robots" content="{{ $entry->seo_robots }}">
        ...
        <div class="relative bg-white">
            ...
            <div class="max-w-screen-lg mx-auto py-12 md:py-24 px-6 flex flex-col gap-6 md:gap-12">
                <div>
                    ...
                    <div class="text-3xl leading-tight md:text-5xl font-semibold w-full">Lettre de résiliation <span class="text-gradient">{{ $entry->name }}</span></div>
                </div>
                <div class="w-full lg:w-4/5 grid grid-cols-1 gap-3">
                    @foreach ($entry->childs as $item)
                        <a
                            href="{{ route('frontend.template.edit', ['slug' => $item->slug]) }}"
                        ...
                        >
                            <div>
                                ...
                                <span class="block text-xl lg:text-27custom font-bold text-gradient">{{ $item->name }}</span>
                            </div>
    ...
    <div class="bg-gray-50">
        <div class="max-w-screen-xl mx-auto py-12 md:py-24 px-6 flex justify-between flex-wrap">
            <div class="text-center md:text-left w-full text-xl md:text-2xl mb-4 md:mb-0 leading-none">Pourquoi choisir resifacile.fr ?</div>
    ...
    <div class="bg-white">
        <div class="max-w-screen-lg mx-auto py-12 md:py-24 px-6 flex flex-col gap-6 md:gap-12">
            <article class="article w-full md:w-4/5">
                {!! Illuminate\Mail\Markdown::parse($entry->article) !!}
            </article>
            <div class="col-span-2">
                <div class="self-start overflow-hidden rounded-xl relative shadow-xl bg-white w-full p-6">
                    ...
                    <div class="flex flex-col text-sm divide-y divide-gray-100">
                        @foreach($entry->marks as $template)
